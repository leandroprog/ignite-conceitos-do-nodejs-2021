...
function checksExistsUserAccount(request, response, next) {
  ...
  if(!userExist) return response.status(400).json({ error: "User not exists"});
...
function checksNotExistsUserAccount(request, response, next) {
  ...
  if(userExist !== -1) return response.status(400).json({ error: "User exist"});
...
app.delete('/todos/:id', checksExistsUserAccount, (request, response) => {
  ...
  if(indexTodo === -1) return response.status(404).json({ error: "Todo not exists"});
